...
interface BookServices {
    ...
    @GET("/api/book/list")
    fun getBooks(): Deferred<Response<BookListResponse>>

    @POST("/api/book/add")
    fun createBook(@Body book : BookRequest) : Deferred<Response<BookResponse>>

    @POST ("/api/book/update")
    fun updateBook(@Body book : Book) : Deferred<Response<BookResponse>>
}